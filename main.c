...
*/
	...
	for(i = 0; i < count; i++){
		...
		if(current_distance < shortest_distance){
			...
			// printf("\n\n");
			// for(q = 0; q < count; q++)
				// printf("Best: %d\tCurrent: %d\n", best_city_list[q].city, current_city_list[q].city);
			// printf("\n\n");
		...
		if(     ( ((double)(t2 - t1))/CLOCKS_PER_SEC) > 5.00     ){
			timer1 += 5;
			printf("Roughly %d seconds...\n", timer1);
			t1 = clock();
		}
		// end = clock();
		...
		if(timer1 >= 120){
			printf("Due to the inaccuracy of the clock() function, we have decided to operate within a 25 - 35 second safety margin.  Program ending...\n");
			break;
		}
